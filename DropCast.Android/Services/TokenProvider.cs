...
public static class TokenProvider
{
    ...
    /// <summary>Path to the encrypted token in Android app-local storage.</summary>
    public static string GetTokenFilePath() =>
        Path.Combine(FileSystem.AppDataDirectory, "token.enc");

    /// <summary>
    /// If no local token.enc exists, try to extract the one bundled in
    /// Resources/Raw (MauiAsset) so the app can auto-connect on first launch.
    /// </summary>
    public static async Task EnsureTokenFromBundleAsync()
    {
        string dest = GetTokenFilePath();
        if (File.Exists(dest)) return;

        try
        {
            using var stream = await FileSystem.OpenAppPackageFileAsync("token.enc");
            using var fs = File.Create(dest);
            await stream.CopyToAsync(fs);
        }
        catch (FileNotFoundException)
        {
            // No bundled token.enc — the user will need to enter the token manually.
        }
    }
}
